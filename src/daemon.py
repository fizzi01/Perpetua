...
class Daemon:
    ...
    def _pre_configure(self):
        ...
        # Nothing to backfill on the server side: ``host`` defaults to
        # "0.0.0.0" (advertise on every interface) and the UID is generated by
        # service discovery. The old fallback here wrote socket.gethostname()
        # into ``host``, which is neither a bind address nor an advertisable
        # one, and could never fire anyway since "0.0.0.0" is truthy.
    ...
    async def _handle_set_server_config(self, params: Dict[str, Any]) -> None:
        ...
        try:
            # An absent or blank uid means "not supplied", never "clear it".
            # The uid keys the client certificate mapping: dropping it forces
            # every paired client through OTP pairing again.
            if params.get("uid"):
                self._server_config.uid = params["uid"]
            ...
            # Persist unconditionally: gating on ``self._server`` meant the GUI
            # was told "Options saved" while nothing reached disk whenever the
            # panel was used before a service had been chosen.
            await self._server_config.save()

            if host_changed and self._server and self._server.is_running():
                # Re-advertising is a courtesy; failing at it must not turn a
                # successful save into an error the user cannot act on.
                try:
                    await self._server.refresh_advertisement()
                except Exception as e:  # noqa: BLE001
                    self._logger.warning(
                        "Could not refresh advertisement after config change",
                        error=str(e),
                    )
    ...
    @CommandHandler.register(DaemonCommand.LIST_NETWORK_INTERFACES)
    async def _handle_list_network_interfaces(self, params: Dict[str, Any]) -> None:
        """Enumerate local IPv4 interfaces for the advertise picker.

        The daemon is the only authority on addresses: the GUI must not run
        its own route lookup, which is how it ended up writing the wrong
        interface into the config on multi-homed machines.

        ``selected`` echoes ``ServerConfig.host`` (an interface preference,
        "0.0.0.0" meaning all) and ``advertised`` is what clients will
        actually be told, so the UI can show it rather than guess.
        """
        command = DaemonCommand.LIST_NETWORK_INTERFACES.value
        try:
            from utils.net import list_local_interfaces_async

            # Include link-local: a direct cable with no DHCP lands on
            # 169.254/16, and omitting it would hide the very interface a
            # user in that setup needs to pick.
            interfaces = await list_local_interfaces_async(include_unusable=True)
            selected = self._server_config.host if self._server_config else None
            advertised = (
                self._server_config.get_advertise_addresses()
