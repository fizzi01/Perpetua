...
The OTP is just for the initial certificate exchange. During pairing the server also issues the client its own certificate (signed by the server's CA and bound to the client's UID). On every later connection the two sides perform **mutual TLS**: the client verifies the server, and the server verifies the client's certificate and checks that its identity matches the claimed UID. This is what prevents another machine on the LAN from impersonating a client by reusing its UID - without the client's private key, the handshake is rejected. The OTP itself never travels over the network - it is shown only on the server's screen.
