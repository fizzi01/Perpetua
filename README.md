...
The OTP is just for the initial certificate exchange. During pairing the server **assigns the client a unique UID** and issues it its own certificate (signed by the server's CA, with that server-assigned UID as the identity); the client's private key never leaves the client, and the UID is delivered only inside the encrypted certificate - it never travels in cleartext. On every later connection the two sides perform **mutual TLS**: the client verifies the server, and the server verifies the client's certificate and checks that its identity matches the claimed UID. This is what prevents another machine on the LAN from impersonating a client - it can neither reuse a UID (the server assigns them and won't sign one you choose) nor connect without the client's private key. The OTP itself never travels over the network - it is shown only on the server's screen.

> **Security note:** the identity guarantee above depends on TLS. If you disable **Secure connection**, the daemon runs in a best-effort, plaintext mode: clients are authenticated only by their self-asserted UID/hostname and the IP allowlist, and an attacker on the same LAN can impersonate a client. Leave `Secure connection` enabled (the default) unless you fully trust the network. Clients paired before this change must re-pair once to obtain a client certificate.
