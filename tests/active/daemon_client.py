...
class DaemonClient:
    ...
    async def execute_command(self, command: str, params: Optional[dict] = None, verbose: bool = False):
...
Examples:
  ...
  %(prog)s discover --timeout 5
    ...
    set_server_parser.add_argument("--heartbeat-interval", type=int, help="Heartbeat interval")
    set_server_parser.add_argument("--ssl-enabled", action="store_true", help="Enable SSL")
    set_server_parser.add_argument("--no-ssl", dest="ssl_enabled", action="store_false", help="Disable SSL")
    ...
    set_client_parser.add_argument("--server-port", type=int, help="Server port to connect to")
    set_client_parser.add_argument("--hostname", help="Client hostname")
    set_client_parser.add_argument("--heartbeat-interval", type=int, help="Heartbeat interval")
    set_client_parser.add_argument("--auto-reconnect", action="store_true", help="Enable auto-reconnect")
    set_client_parser.add_argument("--no-auto-reconnect", dest="auto_reconnect", action="store_false", help="Disable auto-reconnect")
    set_client_parser.add_argument("--ssl-enabled", action="store_true", help="Enable SSL")
    set_client_parser.add_argument("--no-ssl", dest="ssl_enabled", action="store_false", help="Disable SSL")
    set_client_parser.add_argument("--log-level", type=int, help="Log level")
    
    save_config_parser = subparsers.add_parser("save-config", help="Save configuration to disk")
    ...
    reload_config_parser = subparsers.add_parser("reload-config", help="Reload configuration from disk")
    ...
    # Stream management
    enable_stream_parser = subparsers.add_parser("enable-stream", help="Enable a stream")
    ...
    disable_stream_parser = subparsers.add_parser("disable-stream", help="Disable a stream")
    ...
    get_streams_parser = subparsers.add_parser("get-streams", help="Get stream information")
    ...
    add_client_parser = subparsers.add_parser("add-client", help="Add a client (server only)")
    ...
    remove_client_parser = subparsers.add_parser("remove-client", help="Remove a client (server only)")
    ...
    edit_client_parser = subparsers.add_parser("edit-client", help="Edit a client (server only)")
    ...
    share_cert_parser = subparsers.add_parser("share-cert", help="Share certificate (server only)")
    share_cert_parser.add_argument("--host", help="Host address for sharing")
    
    receive_cert_parser = subparsers.add_parser("receive-cert", help="Receive certificate (client only)")
    receive_cert_parser.add_argument(
        "--otp",
        required=True,
        help="6-digit OTP from server"
    )
    
    # Server selection (client only)
    subparsers.add_parser("check-server-choice", help="Check if server choice is needed (client)")
    subparsers.add_parser("get-found-servers", help="Get list of found servers (client)")
    
    choose_server_parser = subparsers.add_parser("choose-server", help="Choose a server (client)")
    choose_server_parser.add_argument(
        "--uid",
        required=True,
        help="UID of the server to choose"
    )
    
    subparsers.add_parser("check-otp", help="Check if OTP is needed (client)")
    
    set_otp_parser = subparsers.add_parser("set-otp", help="Set OTP for certificate (client)")
    ...
    # Service discovery
    discover_parser = subparsers.add_parser("discover", help="Discover services on network")
